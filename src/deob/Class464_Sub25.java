package deob;

public class Class464_Sub25 extends Class464 {

	public static int anInt8715 = 2;

	public static int anInt8716 = 1;

	public static int anInt8717 = 0;

	public void method5722(int arg0, byte arg1) {
		try {
			this.anInt5587 = arg0;
		} catch (RuntimeException var4) {
			throw Class476.method5964(var4, "afw.p(" + ')');
		}
	}

	public Class464_Sub25(int arg0, Class330_Sub50 arg1) {
		super(arg0, arg1);
	}

	public Class464_Sub25(Class330_Sub50 arg0) {
		super(arg0);
	}

	public int method5715(int arg0) {
		try {
			return 1;
		} catch (RuntimeException var3) {
			throw Class476.method5964(var3, "afw.r(" + ')');
		}
	}

	public void method5826(int arg0) {
		try {
			if (this.anInt5587 < 0 && this.anInt5587 > 2) {
				this.anInt5587 = this.method5715(500699457);
			}
		} catch (RuntimeException var3) {
			throw Class476.method5964(var3, "afw.l(" + ')');
		}
	}

	public int method5716(int arg0, int arg1) {
		try {
			return arg0 == 0 || this.aClass330_Sub50_5589.aClass464_Sub27_7881.method5833(16777215) == 1 ? 1 : 2;
		} catch (RuntimeException var4) {
			throw Class476.method5964(var4, "afw.j(" + ')');
		}
	}

	public int method5827(int arg0) {
		try {
			return this.anInt5587;
		} catch (RuntimeException var3) {
			throw Class476.method5964(var3, "afw.k(" + ')');
		}
	}

	public void method5719(int arg0) {
		this.anInt5587 = arg0 * 1558443671 * 295364903;
	}

	public int method5714(int arg0) {
		return arg0 == 0 || this.aClass330_Sub50_5589.aClass464_Sub27_7881.method5833(16777215) == 1 ? 1 : 2;
	}

	public boolean method5828(int arg0) {
		try {
			return true;
		} catch (RuntimeException var3) {
			throw Class476.method5964(var3, "afw.b(" + ')');
		}
	}

	public void method5721(int arg0) {
		this.anInt5587 = arg0 * 1558443671 * 295364903;
	}

	public void method5720(int arg0) {
		this.anInt5587 = arg0 * 1558443671 * 295364903;
	}

	public int method5718() {
		return 1;
	}
}
